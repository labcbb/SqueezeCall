...
from squeeze_call.utils.common import (add_sos_eos, log_add, add_whisper_tokens,
                                mask_to_bias)
from squeeze_call.utils.ctc_utils import remove_duplicates_and_blank, replace_duplicates_with_blank
from squeeze_call.utils.mask import (make_pad_mask, mask_finished_preds,
                              mask_finished_scores, subsequent_mask)
from squeeze_call.utils.context_graph import ContextGraph, ContextState
...
def ctc_greedy_search(ctc_probs: torch.Tensor,
    ...
    for hyp in hyps:
        ...
        r = DecodeResult(replace_duplicates_with_blank(hyp, blank_id))
        results.append(r)
    return results
...
def attention_beam_search(
    ...
    return torch.where(best_hyps != model.eos, best_hyps, 0)
    # results = []
    # for i in range(batch_size):
    #     hyp = best_hyps[i]
    #     hyp = hyp[hyp != model.eos]
    #     if hyp.shape[0] < maxlen:
    #         hyp = torch.nn.functional.pad(hyp, (0, maxlen - hyp.shape[0]))
    #     results.append(hyp)
