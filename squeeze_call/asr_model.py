...
# from basenet.text.base_tokenizer import BaseTokenizer
from squeeze_call.ctc import CTC
from squeeze_call.decoder import TransformerDecoder
from squeeze_call.encoder import SqueezeformerEncoder
from squeeze_call.label_smoothing_loss import LabelSmoothingLoss
from squeeze_call.search import (ctc_greedy_search,
                                      ctc_prefix_beam_search,
                                      attention_beam_search,
                                      attention_rescoring, DecodeResult)
from squeeze_call.utils.mask import make_pad_mask
from squeeze_call.utils.common import (IGNORE_ID, add_sos_eos, th_accuracy,
                                reverse_pad_list)
from squeeze_call.utils.context_graph import ContextGraph
from torchaudio.models.decoder  import ctc_decoder
# from basenet.capi.decode_wrapper_api import DecodeWrapperCApi
...
class ASRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        # if gate_intermediate:
        #     self.encoder.shared_gate_ctc(ctc)

        self.ctc_decoder = ctc_decoder(lexicon=None,
                                        tokens=["<blank>", "A", "C", "G", "T", "<sos>", "<eos>", "<unk>"],
                                        nbest=1,
                                        beam_size=2,
                                        blank_token="<blank>",
                                        unk_word="<unk>",
        ...
        # self.ctc_crf_decoder = DecodeWrapperCApi(256, 1250, self.ctc.odim)
        self.ctc_crf_decoder = None #DecodeWrapperCApi(256, 1200, self.ctc.odim)

        self.alphabet = ["<blank>", "A", "C", "G", "T", "<sos>", "<unk>"]
        self.attention_decoder_weight = 1 - (self.ctc_weight + self.intermediate_ctc_weight)
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out, encoder_mask, intermediate_outputs = self.encoder(speech, speech_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)

        # 2a. CTC branch
        if self.ctc_weight != 0.0:
            loss_ctc, prob_ctc, no_log_prob_ctc = self.ctc(encoder_out, encoder_out_lens, text,
                                           text_lengths)
            # if batch["tag"] == "CV" and prob_ctc.shape[-1] > 1024:
            #     no_log_prob_ctc = self.ctc.decode(prob_ctc.transpose(0, 1))

        else:
            loss_ctc, ctc_probs = None, None

        # 2b. Intermediate CTC branch
        if self.intermediate_ctc_weight !=0 and intermediate_outputs:
            loss_intermediate_ctc = 0.0
            for intermediate_output in intermediate_outputs:
                loss_inter, _, _ = self.ctc(
                    intermediate_output, encoder_out_lens, text, text_lengths
                )
                if torch.is_tensor(loss_inter):
                    loss_intermediate_ctc += loss_inter.sum()
                else:
                    loss_intermediate_ctc += loss_inter.sum()
            loss_intermediate_ctc /= len(intermediate_outputs)
        else:
        ...
        if self.attention_decoder_weight != 0.0:
        ...
        if loss_ctc is not None:
            loss = self.ctc_weight * loss_ctc.sum()

        if loss_intermediate_ctc is not None:
            loss = loss + self.intermediate_ctc_weight * loss_intermediate_ctc

        if loss_att is not None:
            if prob_ctc.shape[-1] > 1024:
                loss_att = loss_att * 0.1
        ...
        return {
            ...
            'prob_ctc': prob_ctc,
            'th_accuracy': acc_att,
            'no_log_prob_ctc': no_log_prob_ctc,
        }
    ...
    @torch.jit.ignore(drop=True)
    def _viterbi(
            self, ctc_probs: torch.Tensor) -> torch.Tensor:
        return self.ctc.viterbi(ctc_probs)
    ...
    def ctc_logprobs(self,
        ...
        ctc_no_log_probs = logits.softmax(dim=2)
        return ctc_probs, ctc_no_log_probs
    ...
    def decode(
        ...
        ctc_probs, ctc_no_log_probs = self.ctc_logprobs(encoder_out, blank_penalty, blank_id)
        results = {}
        ...
        if 'ctc_beam_search' in methods:
            hypothesis = self.ctc_decoder(ctc_probs.cpu(), encoder_lens.cpu())
            result = torch.zeros((ctc_probs.shape[0], encoder_lens.max().item()), dtype=torch.int64)
            for batch_idx, batch_res in enumerate(hypothesis):
                timestep = batch_res[0].timesteps[1:-1] - 1
                result[batch_idx].scatter_(0, timestep.to(torch.int64), batch_res[0].tokens[1:-1])
            results['ctc_beam_search'] = result
        ...
        if 'ctc_prefix_beam_search' in methods:
            ...
            result = torch.zeros((ctc_probs.shape[0], ctc_probs.shape[1]), dtype=torch.int64)
            for batch_idx in range(ctc_no_log_probs.shape[0]):
                seq, timestep = beam_search(ctc_no_log_probs[batch_idx].contiguous().cpu().numpy(), self.alphabet, beam_size=5,
                                beam_cut_threshold=1e-3)
                if len(timestep) > 0:
                    result[batch_idx].scatter_(0, torch.tensor(timestep), torch.tensor(tokenizer.tokens2ids([item for item in seq])))
            results['ctc_prefix_beam_search'] = result

        if 'ctc_crf_prefix_beam_search' in methods:
            move, seq, qstring = self.ctc_crf_decoder(ctc_probs.to(torch.float32))
            results['ctc_crf_prefix_beam_search'] = seq
    ...
    def forward_attention_decoder(
        ...
        #   Issue: https://github.com/basenet-e2e/basenet/issues/1113
        # Equal to:
