...
def float64(func):
    return lambda *args: func(
        *[
            x.to(torch.float64) if hasattr(x, "dtype") and x.dtype is torch.float else x
            for x in args
        ]
    )
...
def benchmark_fwd_bwd(fwd_impl, *inputs, warmup=5, nloops=20):
    ...
    fwd_times, bwd_times = map(
        np.array, zip(*[fwd_bwd_times(fwd_impl, *inputs) for _ in range(nloops)])
    )
...
def add_checks(raw_kernel):
    def wrapped(grid, block, args, *, shared_mem=0):
        ...
        if np.prod(block) > MAX_THREADS:
            raise Exception(
                "Block of size {} not allowed. Maximum number of threads allowed per block is {}.".format(
                    block, MAX_THREADS
                )
            )
...
def load_cupy_func(fname, name, **kwargs):
    try:
        fname = str((Path(__file__).parent / fname).resolve())
    except:
        pass
