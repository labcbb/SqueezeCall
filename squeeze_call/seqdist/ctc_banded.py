...
def window_start_positions(base_alignments, input_lengths, width, num_states):
    ...
    base_alignments = torch.where(
        t[:, None] < input_lengths, base_alignments, num_states - 1
    )
    return torch.min(
        torch.max(base_alignments - width // 2, torch.zeros_like(base_alignments)),
        (num_states - width),
    )


def loss_masked(logits, targets, input_lengths, target_lengths, base_alignments, width):
    state_scores, repeat_mask, final_states, input_lengths = ctc.prepare_inputs(
        logits.log_softmax(2), targets, input_lengths, target_lengths
    )
    T, N, Lp = state_scores.shape
    window_starts = window_start_positions(
        base_alignments, input_lengths, width, target_lengths * 2 + 1
    ).unsqueeze(-1)
    states = torch.arange(Lp, device=logits.device)
    state_scores = torch.where(
        (window_starts <= states) & (states < window_starts + width),
        state_scores,
        logits.new_full((1,), Log.zero),
    )
...
# Cell
def prepare_inputs(
    scores, targets, input_lengths, target_lengths, base_alignments, width: int
):
    (T, N, C), device = scores.shape, scores.device
    num_states = target_lengths * 2 + 1
    states = ctc.interleave_blanks(targets, blank_idx=0)
    repeat_mask = torch.nn.functional.pad(
        states[:, 2:] == states[:, :-2], (2, 2), value=0.0
    )
    window_starts = window_start_positions(
        base_alignments, input_lengths, width, num_states
    )
    ...
    final_states = (
        torch.stack([num_states - 2, num_states - 1], 1)
        - window_starts[input_lengths - 1, torch.arange(N), None]
    )
...
def _logz_fwd(
    ...
    for t in range(0, T):
        ...
        a[:, 2:] = S.sum(
            torch.stack(
                [
                    ...
                    a[:, 1:-1],
                    torch.where(
                        repeat_mask.gather(1, prev_pos + range_W), zeros, a[:, :-2]
                    ),
                ]
            ),
            dim=0,
        ...
        alpha[t + 1] = S.mul(
            state_scores[t], a.gather(1, (pos - prev_pos) + range_W[2:])
        )
        prev_pos = pos
...
def loss_basic(logits, targets, input_lengths, target_lengths, base_alignments, width):
    ...
    logz = _logz_fwd(
        *prepare_inputs(
            log_probs, targets, input_lengths, target_lengths, base_alignments, width
        )
    )
    return -(logz / target_lengths).mean()
...
def fwd_bwd(
    ...
    alpha[0, :, 0] = S.one
    beta[input_lengths, torch.arange(N)] = state_scores.new_full(
        (N, W), S.zero
    ).scatter_(1, final_states, S.one)
    window_starts = torch.cat([window_starts.new_zeros(1, N), window_starts], dim=0)
    alpha_T = fwd_bwd_impl(
        alpha, beta, state_scores, repeat_mask, input_lengths, window_starts, S
    )
...
def _fwd_bwd_py(
    ...
    for t in range(T, 0, -1):
        b = torch.cat(
            [zeros[:, :2], S.mul(beta[t], state_scores[t - 1]), zeros[:, :2]], dim=1
        )
        pos = window_starts[t - 1, :, None]
        b[:, :-2] = S.sum(
            torch.stack(
                [
                    ...
                    b[:, 1:-1],
                    torch.where(
                        repeat_mask.gather(1, prev_pos + range_W), zeros, b[:, 2:]
                    ),
                ]
            ),
            dim=0,
        )
        beta[t - 1, t <= input_lengths] = b.gather(1, (pos - prev_pos) + range_W[2:])[
            t <= input_lengths
        ]
        prev_pos = pos
...
class _Logz(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        g = torch.softmax(alpha[1:] + beta[1:], dim=2) * ctc.masked_grad(
            grad.expand(alpha.size(0) - 1, -1), input_lengths
        )
...
def _fwd_bwd_cupy(
    ...
    with cp.cuda.Device(state_scores.device.index):
        cupy_funcs[(state_scores.dtype, S)](
            grid=(N, 2, 1),
            block=(W, 1, 1),
            shared_mem=2 * 8 * W,
            args=(
                ...
                alpha.data_ptr(),
                beta.data_ptr(),
                state_scores.data_ptr(),
                repeat_mask.data_ptr(),
                input_lengths.data_ptr(),
                window_starts.data_ptr(),
                N,
                L,
                W,
            ),
...
# Cell
cupy_funcs[(torch.float32, Max)] = load_cupy_func(
    "cuda/ctc_banded.cu",
    "fwd_bwd_banded",
    FLOAT="float",
    SUM="max3",
...
cupy_funcs[(torch.float64, Max)] = load_cupy_func(
    "cuda/ctc_banded.cu",
    "fwd_bwd_banded",
    FLOAT="double",
    SUM="max3",
...
class _LogzViterbi(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        alpha, beta, input_lengths = ctx.saved_tensors
        g = Max.dsum(alpha[1:] + beta[1:], dim=2) * ctc.masked_grad(
            grad.expand(alpha.size(0) - 1, -1), input_lengths
        )
        return g, None, None, None, None, None
...
def soft_alignments(
    logits, targets, input_lengths, target_lengths, base_alignments, width
):
    state_scores, *args = prepare_inputs(
        logits.log_softmax(2),
        targets,
        input_lengths,
        target_lengths,
    ...
    _Logz.apply(
        state_scores.detach_().requires_grad_(), *args, _fwd_bwd_cupy
    ).sum().backward()
    return state_scores.grad


def viterbi_alignments(
    logits, targets, input_lengths, target_lengths, base_alignments, width
):
    state_scores, *args = prepare_inputs(
        logits.log_softmax(2),
