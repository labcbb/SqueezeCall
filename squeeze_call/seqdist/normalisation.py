...
def generate_test_example(T, N, n_state, dtype=torch.float):
    return torch.rand(
        (T, N, n_state, n_state), device=device, dtype=dtype, requires_grad=True
    )
...
cupy_funcs = {
    (torch.float32, Log): load_cupy_func(
        "cuda/fused_bmv.cu", "fwd", FLOAT="float", MUL="add", SUM="logsumexp2"
    ),
    (torch.float64, Log): load_cupy_func(
        "cuda/fused_bmv.cu", "fwd", FLOAT="double", MUL="add", SUM="logsumexp2"
    ),
    (torch.float32, Max): load_cupy_func(
        "cuda/fused_bmv.cu", "fwd", FLOAT="float", MUL="add", SUM="max2"
    ),
    (torch.float64, Max): load_cupy_func(
        "cuda/fused_bmv.cu", "fwd", FLOAT="double", MUL="add", SUM="max2"
    ),
}
