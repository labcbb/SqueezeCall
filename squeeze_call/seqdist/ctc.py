...
def generate_sample_inputs(T_min, T_max, N, C, L_min, L_max, device=device):
    ...
    input_lengths = torch.randint(
        T_min, T_max + 1, (N,), dtype=torch.long, device=device
    )
    target_lengths = torch.randint(
        L_min, L_max + 1, (N,), dtype=torch.long, device=device
    )
...
def prepare_inputs(scores, targets, input_lengths, target_lengths):
    ...
    repeat_mask = torch.nn.functional.pad(
        states[:, 2:] == states[:, :-2], (2, 0), value=0.0
    )
...
def _logz_fwd(
    state_scores, repeat_mask, final_states, input_lengths, S: semiring = Log
):
    ...
    for t in range(0, T):
        alpha[t] = a[:, 2:] = S.mul(
            ...
            S.sum(
                torch.stack(
                    [a[:, 2:], a[:, 1:-1], torch.where(repeat_mask, zeros, a[:, :-2])]
                ),
                dim=0,
            ),
        )
    return S.sum(
        alpha[input_lengths - 1, torch.arange(N)].gather(1, final_states), dim=1
    )
...
def _fwd_bwd_py(
    alpha, beta, state_scores, repeat_mask, input_lengths, S: semiring = Log
):
    T, N, Lp = state_scores.shape
    ...
    for t in range(T, 0, -1):
        ...
        b[:, :-2] = S.sum(
            torch.stack(
                [b[:, :-2], b[:, 1:-1], torch.where(repeat_mask, zeros, b[:, 2:])]
            ),
            dim=0,
        )
...
class _Logz(torch.autograd.Function):
    ...
    def forward(
        ...
        alpha, beta, logz = fwd_bwd(
            state_scores, repeat_mask, final_states, input_lengths, fwd_bwd_impl, S
        )
    ...
    def backward(ctx, grad):
        ...
        g = ctx.semiring.dsum(alpha[1:] + beta[1:], dim=2) * masked_grad(
...
def loss_py(logits, targets, input_lengths, target_lengths):
    logz = _Logz.apply(
        *prepare_inputs(logits.log_softmax(2), targets, input_lengths, target_lengths),
        _fwd_bwd_py,
        Log
    )
    return -(logz / target_lengths).mean()
...
def viterbi_alignments(logits, targets, input_lengths, target_lengths):
    state_scores, repeat_mask, final_states, input_lengths = prepare_inputs(
        logits.log_softmax(2), targets, input_lengths, target_lengths
    )
    _Logz.apply(
        state_scores.detach_().requires_grad_(),
        repeat_mask,
        final_states,
        input_lengths,
        _fwd_bwd_cupy,
...
def soft_alignments(logits, targets, input_lengths, target_lengths, beta=1.0):
    state_scores, repeat_mask, final_states, input_lengths = prepare_inputs(
        (logits * beta).log_softmax(2), targets, input_lengths, target_lengths
    )
    _Logz.apply(
        state_scores.detach_().requires_grad_(),
        repeat_mask,
...
Prob = semiring(
    ...
    mul=torch.mul,
    sum=torch.sum,
    dsum=(lambda x, dim=0: torch.ones_like),
)
cupy_funcs[(torch.float64, Prob)] = load_cupy_func(
    "cuda/ctc.cu", "fwd_bwd_logspace", FLOAT="double", SUM="sum3", MUL="mul", ZERO="0.0"
)
...
class _LogzDirect(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, state_scores, repeat_mask, final_states, input_lengths, fwd_bwd_impl
    ):
        alpha, beta, z = fwd_bwd(
            state_scores, repeat_mask, final_states, input_lengths, fwd_bwd_impl, Prob
        )
    ...
    def backward(ctx, grad):
        ...
        g = (
            (g / state_probs)
            * masked_grad(grad.expand(alpha.size(0) - 1, -1), input_lengths)
            / (g.sum(-1, keepdim=True) + 1e-38)
        )
        return g, None, None, None, None


def loss_direct_cupy(logits, targets, input_lengths, target_lengths):
    logz = _LogzDirect.apply(
        *prepare_inputs(logits.softmax(2), targets, input_lengths, target_lengths),
        _fwd_bwd_cupy
    )
    return -(logz / target_lengths).mean()
