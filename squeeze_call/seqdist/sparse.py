...
class _LogZ_scan(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        g = S.mul(
            S.mul(Ms.reshape(T, N, -1), alpha[:-1, :, idx.flatten()]).reshape(
                T, N, C, nz
            ),
            beta[:-1, :, :, None].flip(0),
        )
...
def _ctc_loss(
    logits, targets, input_lengths, target_lengths, logZ_impl, S: semiring = Log
):
    ...
    with cp.cuda.Device(Ms.device.index):
        cupy_funcs[(Ms.dtype, S)](
            ...
            args=(
                alpha.data_ptr(),
                Ms.data_ptr(),
                idx.to(dtype=torch.int, device=Ms.device).data_ptr(),
                T,
                N,
                C,
                nz,
...
def cupy_func(func_name, dtype, S, NZ, K):
    ...
    return load_cupy_func(
        fname,
        func_name,
        FLOAT=float_types[dtype],
        MUL=ops[S]["mul"],
        ZERO="{:E}".format(S.zero),
        SUM=ops[S]["sum"],
        NZ=NZ,
        K=K,
    )
...
def logZ_fwd_cupy(Ms, idx, v0, vT, S: semiring = Log, K=4):
    ...
    with cp.cuda.Device(Ms.device.index):
        cupy_func("logZ_fwd", Ms.dtype, S, NZ, K)(
            grid=(N, 1, 1),
            block=(C // K, 1, 1),
            shared_mem=2 * _bytes * C,
            args=(
                logZ.data_ptr(),
                Ms_grad.data_ptr(),
                Ms.data_ptr(),
...
def fwd_scores_cupy(Ms, idx, v0, S: semiring = Log, K=4):
    ...
    _bytes = 8 if (Ms.dtype == torch.float64) else 4
    with cp.cuda.Device(Ms.device.index):
        cupy_func("fwd_scores", Ms.dtype, S, NZ, K)(
            grid=(N, 1, 1),
            block=(C // K, 1, 1),
            shared_mem=2 * _bytes * C,
            args=(
...
def bwd_scores_cupy(Ms, idx, vT, S: semiring = Log, K=4):
    ...
    _bytes = 8 if (Ms.dtype == torch.float64) else 4
    with cp.cuda.Device(Ms.device.index):
        cupy_func("bwd_scores", Ms.dtype, S, NZ, K)(
            grid=(N, 1, 1),
            block=(C // K, 1, 1),
            shared_mem=2 * _bytes * C,
            args=(
